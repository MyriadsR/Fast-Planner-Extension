...
void dyn_pb(const vecVec3 &ps, const vecVec3 &vs, const vecVec3 &ss, const ros::Time &ts, const string &world_frame_id)
{
    ...
    for (uint i = 0; i < ps.size(); i++)
    {
        ...
        // 修改为球体类型
        dynobs.type = visualization_msgs::Marker::SPHERE;
        ...
        dynobs.scale.y = ss[i](0);
        dynobs.scale.z = ss[i](0);
...
{
    // Initialize ROS
    ros::init(argc, argv, "fake_obs_sphere_publisher");
    ...
    // DynObsSize_min/max：球体障碍物的最小/最大直径
    ...
    for (int i = 0; i < obs_num; i++)
    {
        ...
        // 为球体生成随机直径（只使用第一个维度），范围在 size_min[0] 与 size_max[0] 之间
        double diameter = (double)(rand() % 101) / 101 * (size_max[0] - size_min[0]) + size_min[0];
        size(0) = diameter;  // 球体只需要一个直径值
        size(1) = diameter;  // 保持一致性
        size(2) = diameter;  // 保持一致性
